...
public class AddAlarm extends AppCompatActivity {
    ...
    private void processinsert(String title, String date, String time) {
        ...
        if (firebaseUser != null) {
            DatabaseReference ref = FirebaseDatabase.getInstance().getReference("Alarms").child(firebaseUser.getUid()).child("Tasks");
            Model model = new Model(title, date, time);
            ref.child(ref.push().getKey()).setValue(model);
        }

        setAlarm(title, date, time);
        //calls the set alarm method to set alarm
        mTitledit.setText("");
        Toast.makeText(this, "Task Alarm Set Successfully", Toast.LENGTH_SHORT).show();
    ...
    public void onBackPressed() {
        startActivity(new Intent(AddAlarm.this, HomeScreen.class));
        finish();
    }
}